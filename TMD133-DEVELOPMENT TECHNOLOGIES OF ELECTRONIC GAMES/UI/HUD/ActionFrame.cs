...
namespace RTS.FDG.UI.HUD
{
    public class ActionFrame : MonoBehaviour
    {
        ...
        private Buildings.BasicBuilding IsBuilding(string name)
        {
            ...
            {
                foreach (Buildings.BasicBuilding buildings in actionsList.basicBuildings)
                {
                    if (buildings.name == name)
                    {
                        return buildings;
                    }
                }
        ...
        {
            GameObject spawnedObject = Instantiate(spawnOrder[0], new Vector3(spawnPoint.transform.position.x,spawnPoint.transform.position.y,spawnPoint.transform.position.z-30),Quaternion.identity);
